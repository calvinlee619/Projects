...
//                new Document("$pushAll",new Document("visited",businessIds)));
        ...
        iterable.forEach(new Block<Document>() {
            ...
            public void apply(final Document document) {
                if (document.containsKey("categories")) {
                    String[] list = document.getString("categories").split(",");
    ...
    public JSONArray searchRestaurants(String searchName, String location, String userId) {
        try {
            ...
            String response = api.searchForBusinessesByLocation(searchName,location);
            List<JSONObject> list = this.storeInDatabase(response,userId);
    ...
    public JSONArray searchRestaurants(String userId, double lat, double lon) {
        try {
            YelpAPI api = new YelpAPI();
            String response = api.searchForBusinessesByLocation(lat, lon);
            List<JSONObject> list = this.storeInDatabase(response,userId);
            return new JSONArray(list);
        } catch (Exception e) {
            System.out.println(e.getMessage());
    ...
    private List<JSONObject> storeInDatabase(String response, String userId) throws JSONException {
        List<JSONObject> list = new ArrayList<JSONObject>();
        Set<String> visited = new HashSet<>();
        if (userId != null)  visited = getVisitedRestaurants(userId);

        JSONObject root = new JSONObject(response);
        JSONArray array = root.getJSONArray("businesses");
